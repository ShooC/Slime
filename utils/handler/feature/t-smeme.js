...
export const handle = async (m, { q, conn, mime, quoted }) => {
	...
	let name = {name: 'Original By', author: 'Slime Bot'}
	if (!bawah) {
		if (/webp/.test(mime)) {
	...
	} else conn.sendteks(m.chat, `⛱️Balas stiker dengan teks *.${m.command} teks kesatu , teks kedua*\nAtau Balas gambar / kirim gambar dengan caption *.${m.command} teks kesatu , teks kedua*`, m)
}